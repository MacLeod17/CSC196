...
namespace gk
{
	...
	void Player::Update(float dt)
	{
		...
		if (Core::Input::IsPressed('W')) { force = gk::Vector2::forward * m_thrust; }
		force = gk::Vector2::rotate(force, m_transform.angle);
		...
		//Rotate
		if (Core::Input::IsPressed('A')) { m_transform.angle -= (dt * gk::DegreesToRadians(360.0f)); }
		if (Core::Input::IsPressed('D')) { m_transform.angle += (dt * gk::DegreesToRadians(360.0f)); }
